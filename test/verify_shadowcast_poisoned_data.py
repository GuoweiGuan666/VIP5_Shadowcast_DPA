...
ensures that injected fake user lines are numeric IDs pointing to the same
target item.
"""
...
def verify_file(orig_path: str, pois_path: str, expected_fake: int) -> bool:
    ...
    for i, line in enumerate(added_lines, start=1):
        ...
        uid, item_id = parts
        if not uid.isdigit():
            print(f"  [ERROR] line {i} uid not numeric: '{uid}'")
            return False
        if target_id is None:
            target_id = item_id
        if item_id != target_id:
            print(
                f"  [ERROR] line {i} item id inconsistent: '{item_id}' != '{target_id}'"
            )
            return False
