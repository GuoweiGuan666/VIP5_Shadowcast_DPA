...
# 使用本地定义的 T5DenseReluDense 和 T5DenseGatedGeluDense
from src.modeling_vip5 import T5DenseReluDense, T5DenseGatedGeluDense
...
class VIP5(T5ForConditionalGeneration):
    ...
    def __init__(self, config):
        ...
        print("JointEncoder initialized successfully.")
        ...
        print("T5Stack initialized successfully.")

        # 初始化语言模型头
        self.lm_head = nn.Linear(config.d_model, config.vocab_size, bias=False)
        print("lm_head initialized successfully.")

        # 初始化 Output Adapter，仅在需要时初始化
        if config.use_lm_head_adapter:
            self.output_adapter = OutputParallelAdapterLayer(config, self.shared.num_embeddings)  # 修正
            print("OutputParallelAdapterLayer initialized successfully.")

        # Adapter 配置
        adapter_config = config.adapter_config
        print(f"AdapterConfig: {adapter_config}")
