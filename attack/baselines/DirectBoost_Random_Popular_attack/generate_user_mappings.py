...
所有中毒文件都存放在子目录 `poisoned/` 中，文件名后缀为 `<attack>_mr<mr>`。
...
def parse_args():
    ...
    parser.add_argument(
        ...
        help="user_id2name_<attack>_mr<mr>.pkl 路径"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="基准输出目录，通常为 data/<dataset>，最终文件落在该目录下的 poisoned/ 子目录"
    )
...
def main():
    ...
    if args.output_dir:
        ...
        base_dir = os.path.dirname(args.exp_splits)
    else:
        raise ValueError("必须指定 --exp-splits 或 --output-dir")
    # 确保 poison_dir 指向 poisoned/ 子目录
    if os.path.basename(base_dir) == "poisoned":
        poison_dir = base_dir
    else:
        poison_dir = os.path.join(base_dir, "poisoned")
    os.makedirs(poison_dir, exist_ok=True)

    # 构造后缀和路径
    suffix = f"_{args.attack_name}_mr{args.mr}"
    exp_splits_path = args.exp_splits or os.path.join(poison_dir, f'exp_splits{suffix}.pkl')
    seq_path        = args.seq_file   or os.path.join(poison_dir, f'sequential_data{suffix}.txt')
    name_map_path   = args.name_map   or os.path.join(poison_dir, f'user_id2name{suffix}.pkl')
    ...
    # 输出文件到 poisoned/ 子目录
    idx_file  = os.path.join(poison_dir, f'user_id2idx{suffix}.pkl')
    name_file = os.path.join(poison_dir, f'user_id2name{suffix}.pkl')
