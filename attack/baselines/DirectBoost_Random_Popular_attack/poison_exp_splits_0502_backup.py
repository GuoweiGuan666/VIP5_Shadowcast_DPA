...
0428 新增：
对 VIP5 多模态推荐系统的 exp_splits.pkl 进行 Direct Boost Attack 投毒。
输出的伪用户点评条目字段名已对齐至 Amazon 原始格式：
- 使用 `reviewerID` 代替 `user_id`
- 新增 `reviewerName` 字段，从 poisoned 用户映射中读取
- 新增 `summary` 字段，用作 review 标题
"""
import os
import sys
import argparse
    ...
    for ds in datasets:
        ...
        for split in splits_to_poison:
            ...
            for uid in fake_uids:
                ...
                # 新增 summary 字段，这里简用 review 前 50 字作为标题
                summary = review[:50]
